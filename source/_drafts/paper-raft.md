...
[raft-03](paper-raft/raft-03.png)

* 在一个任期以内最多有一位领导者

* 领导者不会去更改或者删除条目中的日志，它们只会添加新的日志

* 如果一个日志条目在某一个任期内被提交，那么该条目将在所有更高任期的领导者的日志中存在

* 如果一个服务器在给定索引处将一个日志条目应用到其状态机，那么没有其他服务器会在相同的索引出应用不同的日志条目
...
`Raft`对于日志管理会保持两条日志匹配属性：

* 如果两个不同日志中的条目具有相同的索引和任期，则它们存储相同的命令。

* 如果两个不同日志中的条目具有相同的所有和任期，则这些日志在给定索引之前的所有条目中都是相同的。

第一个属性源于以下事实：领导者在给定任期内之多创建一个具有给定日志所有的条目，并且日志条目永远不会改变其在日志中的位置。第二个属性通过`AppendEntries`执行简单一致性检查来保证。当发送`AppendEntries RPC`时，领导者包括日志中紧接新条目之前的条目的索引和任期。如果追随者在其日志中找不到具有相同索引和任期的条目，它将拒绝新条目。

很显然在正常操作的期间，领导者和追随者日志保持一致，因此`AppendEntries`的一致性检查永远不会失败，然而，我们的领导者可能出现奔溃宕机的场景，所有就存在旧的领导者和新的领导者日志不一致的现象，

[raft-07](paper-raft/raft-07.png)

在上图中的`f`可能发生的场景就是，在任期`2`内，`f`为领导者，此时追加了3个条目，然后在提交任何条目之前崩溃；它很快重新启动，成为任期3的领导者，并在其日志中添加了更多条目，在任期2或任期3中的任何条目提交之前，该服务器再次崩溃并在多个任期内保持宕机。这就导致了不一致性的问题。

在`raft`中，领导者通过强制追随者日志复制其日志来处理不一致，这意味着追随者日志中的冲突条目将被领导者日志中的条目覆盖。为了使得追随者日志与领导者日志保持一致，领导者会先找到两者日志一致的最新日志条目，删除追随者日志该点以后的所有条目，并将该点之后的所有领导者日志发送给追随者。所有这些操作都在`AppendEntries RPC`的一致性检查中进行。

领导者为每个追随者维护一个`nextIndex`，即领导者将发送给追随者的下一个日志条目的是索引，当领导者首次上任时，它将所有`nextIndex`值初始化为日志中最后一个条目之后的索引。

如果追随者的日志与领导者不一致，则下一次`AppendEntries RPC`中的一致性检查将失败，拒绝后，领导者会递减`nextIndex`并重试`AppendEntries RPC`，最终，`nextIndex`将到达领导者和追随者日志匹配的点。随后便可以进行追加保持一致。我们可以看到会有一种最坏的情况，就是`nextIndex`减小到了`0`，也就是追随者要全盘复制领导者的日志条目。

### 安全性

在基于我们前面的算法中，可能会存在这样的问题，假设一个追随者在领导者提交多个日志条目时可能不可用，也就是它接收不到领导者的条目，但是随后它可能会被选为领导者，并会用新条目覆盖这些日志条目，结果就是，不同状态机可能会执行不同的命令序列。而通过对领导者的选举施加一定的限制可以完善这个问题。

#### 选举限制

在任何基于领导者的公式算法中，领导者最终必须存储所有已提交的日志条目。`Raft`使用投票的过程来防止那些未包含所有提交条目的候选者赢得选举。候选者必须联系集群中的多数成员才能当选，这意味着每个提交的条目必须至少存在于这些服务器中的一个。如果候选者的日志至少与该多数中任何其他日志一样新，则它将包含所有提交的条目。`RequestVote RPC`实现了这一限制：`RPC`包含有关候选者日志的信息，如果选民的日志比候选者的日志更新，它将拒绝投票。

`Raft`通过比较日志中最后一个条目的索引和任期来确定两个日志哪个更新。如果日志的最后一个条目具有不同的任期，则任期较晚的日志更新。如果日志以相同的任期结束，则日志更长的日志更新。

#### 提交先前任期的条目

当领导者知道一个来自当前任期的条目已经存储在多数服务器上时，该条目就被认为时提交的。如果领导者在提交一个条目之前崩溃，未来的领导者将尝试完成该条目的复制。然而，领导者无法立即得出结论，即存储在多数服务器上的先前任期的条目已经提交。
