...
在`GFS`中，主节点执行所有命名空间操作，同时，还管理着整个系统的块副本：做出放置决策、创建新块以及协调各种系统范围的活动，以保持块完全复制、平衡所有块服务器的负载，并回收未使用的存储。下面会分别介绍主节点的一些控制操作。

## 命名空间的管理和锁定。

在`GFS`中，会在逻辑上将命名空间表示为一个查找表，将完整路径名映射到元数据，通过前缀压缩，这本表可以高效的表示在内存中，命名空间树中
